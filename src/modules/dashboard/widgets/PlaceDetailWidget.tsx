...
export function PlaceDetailWidget() {
  ...
  return (
    <div className="h-full w-full bg-emerald-50/60 rounded-xl flex flex-col overflow-hidden">
      <div className="px-3 py-2 bg-emerald-100/40 flex items-center gap-2">
        <MapPin className="h-4 w-4 shrink-0" style={{ color: "#10B981" }} />
        <span className="text-sm font-semibold truncate flex-1">{title}</span>
        <button
          type="button"
          onClick={() => select(null)}
          className="shrink-0 p-1 rounded hover:bg-emerald-200/50 transition-colors"
